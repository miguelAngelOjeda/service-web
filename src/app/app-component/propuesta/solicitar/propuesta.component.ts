...
export class PropuestaComponent implements OnInit {
  ...
  calcularCuota() {
    ...
    /*console.log('Gastos admin: '+ gastosAdmin);
    console.log('Seguro Vida: '+ seguroVida);
    console.log('dividendoModalidad: '+ dividendoModalidad);*/
    ...
    for(let i = 1; i <= Number(plazo); i++) {
      ...
      //depende de la modalidad para adicionar los meses, dias, etc
  ...
  printLiquidacion(){
    ...
    pdf.add( 
      new Columns([new Txt('Sucursal: ' + 'ITAGUA').alignment('left').end, new Txt('Plazo: ' + this.myForm.get('plazo').value).alignment('left').end]).end
    ...
    pdf.add(
      new Columns([new Txt('Modalidad: ' + this.myForm.get('modalidad').value.nombre).alignment('left').end, new Txt('T.A.N.: ' + tasaAnual + ' %').alignment('left').end]).end
    ...
    pdf.add(
      new Columns([new Txt('Monto Capital: ' + new Intl.NumberFormat().format(this.myForm.get('montoSol').value)).alignment('left').end, new Txt('Monto Interes: ' + new Intl.NumberFormat().format(totalInteres)).alignment('left').end]).end
