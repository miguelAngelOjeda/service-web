    ...
    merge(this.sort.sortChange, this.paginator.page, fromEvent(this.filterInput.nativeElement, 'keyup'))
        .pipe(
          ...
          switchMap(() => {
            ...
            return this.apiService.getPageList('/tipos-desembolsos',this.isfilter,this.filterInput.nativeElement.value, this.rulesColumns,
          ...
          map(data => {
            ...
            return data.rows as OutlaysTypes[];;
          }),
          catchError(() => {
            return observableOf([]);
          })
  ...
  delete(data: any){
    if(data.id){
      ...
      dialogRef.afterClosed().subscribe(result => {
        if(result){
          this.apiService.delete('/tipos-desembolsos/' + data.id)
          .subscribe(res => {
              if(res.status == 200){
