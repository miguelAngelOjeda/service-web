...
@Component({
  selector: 'app-list-tryst-types',
  templateUrl: './list-tryst-types.component.html',
  styleUrls: ['./list-tryst-types.component.scss']
})
export class ListTrystTypesComponent implements AfterViewInit {
    ...
    merge(this.sort.sortChange, this.paginator.page, fromEvent(this.filterInput.nativeElement, 'keyup'))
        .pipe(
          ...
          switchMap(() => {
            ...
            return this.apiService.getPageList('/tipos-citas',this.isfilter,this.filterInput.nativeElement.value, this.rulesColumns,
  ...
  delete(data: any){
    if(data.id){
      ...
      dialogRef.afterClosed().subscribe(result => {
        if(result){
          this.apiService.delete('/tipos-citas/' + data.id)
          .subscribe(res => {
              if(res.status == 200){
