...
import { IngressTypes, Filter, Rules, Message  } from '../../../../core/models';
    ...
    merge(this.sort.sortChange, this.paginator.page, fromEvent(this.filterInput.nativeElement, 'keyup'))
        .pipe(
          ...
          switchMap(() => {
            ...
            return this.apiService.getPageList('/tipos-ingresos',this.isfilter,this.filterInput.nativeElement.value, this.rulesColumns,
  ...
  delete(data: any){
    if(data.id){
      ...
      dialogRef.afterClosed().subscribe(result => {
        if(result){
          this.apiService.delete('/tipos-ingresos/' + data.id)
          .subscribe(res => {
              if(res.status == 200){
