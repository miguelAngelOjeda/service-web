...
@Component({
  selector: 'app-list-horary-types',
  templateUrl: './list-horary-types.component.html',
  styleUrls: ['./list-horary-types.component.scss']
})
export class ListHoraryTypesComponent implements AfterViewInit {
    ...
    merge(this.sort.sortChange, this.paginator.page, fromEvent(this.filterInput.nativeElement, 'keyup'))
        .pipe(
          ...
          switchMap(() => {
            ...
            return this.apiService.getPageList('/tipos-estudios',this.isfilter,this.filterInput.nativeElement.value, this.rulesColumns,
  ...
  delete(data: any){
    if(data.id){
      ...
      dialogRef.afterClosed().subscribe(result => {
        if(result){
          this.apiService.delete('/tipos-estudios/' + data.id)
          .subscribe(res => {
              if(res.status == 200){
