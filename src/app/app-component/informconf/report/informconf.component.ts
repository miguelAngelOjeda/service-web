...
export class InformconfComponent implements OnInit{
  ...
  onSubmit() {
    let variables = new HttpParams({fromObject :
      {'tipoPersona' : this.filterForm.get('tipoPersona').value,'historico' : 'false','documento' : this.filterForm.get('documento').value}});

    this.apiService.get('/informconf/reporte',variables)
    .subscribe(res => {
      if(res.status == 200){
        this.loadValue(res.model);
      }
    });
  }
