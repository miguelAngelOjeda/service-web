...
export class ListMessageComponent implements OnInit {
  ...
  public displayedColumns = ['codPersona','cliente','monto', 'atraso', 'fechaVencimiento', 'telefonoParticular', 'mensaje', 'email'];
  public dataSource = new MatTableDataSource<any>();
  public uploaderFile: FileUploader = new FileUploader({isHTML5: true,autoUpload: false});
  ...
  ngOnInit() {
    ...
    this.uploaderFile.onAfterAddingFile = (fileItem) => {
      const formData = new FormData();
      formData.append('file', fileItem._file);

      this.apiService.post('/archivos/message', formData)
      .subscribe(res => {
        if(res.status == 200){
          this.paginator._changePageSize(this.paginator.pageSize);
        }
      });
      this.uploaderFile.clearQueue();
    };
  }
  ...
  filters() {
    ...
    merge(this.sort.sortChange, this.paginator.page, filterInp, filterButton)
        .pipe(
          startWith({}),
          switchMap((event: any) => {
            return this.apiService.getList('/mensajes', this.filterValue, this.rulesColumns,
