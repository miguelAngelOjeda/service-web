...
import { ViewModalPeopleRelationsComponent } from './view-modal-people-relationship';
import { AddModalPeopleRelationsComponent } from './add-modal-people-relationship';
...
@Component({
  ...
  export class  PeopleRelationshipComponent implements OnInit{
    ...
    editRelationship(id: number, index: number) {
        ...
        if(res.status == 200){
          ...
          dialogRef.afterClosed().subscribe(result => {
            if (result) { // <- This is not ok, should check for undefined, not empty string
              result.avatar = null;
              result.fechaNacimiento =  new Date(result.fechaNacimiento);
              result.nombre = (result.primerNombre == null ? '' : result.primerNombre) + ' '
                                  + (result.segundoNombre == null ? '' : result.segundoNombre) + ' '
                                  + (result.primerApellido == null ? '' : result.primerApellido) + ' ' + (result.segundoApellido == null ? '' : result.segundoApellido);

              (<FormGroup>(<FormArray>this.relationshipForm.get(this.formName)).controls[index].get('personaVinculo')).patchValue(result);
            }
          });
        }
      });
    ...
    addRelationship(id: number, index: number) {
      const dialogConfig = new MatDialogConfig();
      dialogConfig.disableClose = true;
      dialogConfig.autoFocus = true;

      const dialogRef = this.dialog.open(AddModalPeopleRelationsComponent, dialogConfig);
    ...
    viewRelationship(id: number) {
      ...
      .subscribe(res => {
        if(res.status == 200){
          ...
          const dialogRef = this.dialog.open(ViewModalPeopleRelationsComponent, dialogConfig);
        }
      });
    }
